...
public final class PlayerFinder {
    ...
    private static final Cache<String, UUID> name2UUIDCache = CacheBuilder.newBuilder()
            .expireAfterAccess(60, TimeUnit.MINUTES).build();
    private static final Cache<UUID, String> uuid2StringCache = CacheBuilder.newBuilder()
            .expireAfterAccess(60, TimeUnit.MINUTES).build();
    ...
    @Nullable
    private static PlayerProfile findProfileByName(String name,
            java.util.Collection<? extends org.bukkit.OfflinePlayer> players, boolean isOfflinePlayer)
    {
    ...
    /**
     * Get the unmodified list of cached offline player names will return empty when
     * useOfflineStash is false
     */
    ...
    public static void doLargeOfflineCachingWork(QuickShop quickShop, OfflinePlayer[] offlinePlayers) {
        ...
        quickShop.getLogger().log(Level.INFO,
                "Large server detected (offline player > 2000), start offline player caching...");
        useOfflineStash = true;
        int amount = 0;
        int errorAmount = 0;
        int doneAmount = 0;
        int reportingInterval = Math.max(1000, offlinePlayers.length / 20); // Reduce the amount of log spam from
                                                                            // caching offline players
        for (OfflinePlayer offlinePlayer : offlinePlayers) { // use the existing offlinePlayers array
            ...
            if (amount == reportingInterval) {
                ...
                amount = 0;
                quickShop.getLogger().log(Level.INFO, "Caching Offline player...cached " + doneAmount + "/"
                        + offlinePlayers.length + " players, " + errorAmount + " errors got when caching players.");

            }

        }

        quickShop.getLogger().log(Level.INFO, "Done! cached " + offlinePlayers.length + " players, " + errorAmount
                + " errors got when caching players.");
     ...
     * @param forceWebRequest whether to use web request, may result blocking server
     *                        thread
     ...
     *
     * @param name                    the player name, case ignored
     * @param includingBlockingWebReq whether to use web request, may result
     *                                blocking server thread
     * @param checkOfflinePlayer      whether to check offline player, beware that
     *                                should be a large cost for some times
     * @return the uuid of this player name, null when [includingBlockingWebReq] is
     *         false
     */
    @Nullable
