...
public class MavenUpdater implements QuickUpdater {
    ...
    public MavenUpdater(BuildInfo pluginBuildInfo) {
        ...
        this.versionType = "PotatoCraft-Studio/QuickShop-Reremake".equals(pluginBuildInfo.getJobName())
                ? VersionType.STABLE
        ...
        try {
            ...
            QuickShop.getInstance().getLogger().log(Level.WARNING,
                    "Could not init sax parser for xml, update checking is not available!", e);
    ...
    public boolean isLatest() {
        ...
        try {
            ...
            if (versionType == VersionType.STABLE) {
                ...
                try (Response response = HttpUtil.getClientInstance()
                        .newCall(new Request.Builder().url(mavenStableBaseUrl + "maven-metadata.xml")
                ...
                try (Response response = HttpUtil.getClientInstance()
                        .newCall(new Request.Builder().url(mavenSnapshotBaseUrl + "maven-metadata.xml")
                                .header("User-Agent", "QuickShop-" + QuickShop.getFork() + " " + QuickShop.getVersion())
                                .get().build())
                        .execute();
                        ResponseBody responseBody = response.body();
                ...
                if (isLatest) {
                    ...
                    try (Response response = HttpUtil.getClientInstance().newCall(new Request.Builder()
                            .url(mavenSnapshotBaseUrl + remoteVersion + "/maven-metadata.xml")
                            .header("User-Agent", "QuickShop-" + QuickShop.getFork() + " " + QuickShop.getVersion())
                            .get().build()).execute();
                            ResponseBody responseBody = response.body();
                            InputStream is = responseBody == null ? null : responseBody.byteStream())
            ...
            MsgUtil.sendDirectMessage(Bukkit.getConsoleSender(), ChatColor.RED
                    + "[QuickShop] Failed to check for an update on build server! It might be an internet issue or the build server host is down. If you want disable the update checker, you can disable in config.yml, but we still high-recommend check for updates on SpigotMC or CodeMC Jenkins often, Error: "
                    + e.getMessage());
    ...
    private byte[] downloadUpdate() throws IOException {
        ...
        try (Response response = HttpUtil.getClientInstance()
                .newCall(new Request.Builder().url(updateBaseUrl + "lastSuccessfulBuild/artifact/target/QuickShop.jar")
                        .header("User-Agent", "QuickShop-" + QuickShop.getFork() + " " + QuickShop.getVersion()).get()
                        .build())
                .execute();
                ResponseBody responseBody = response.body();
                InputStream is = responseBody == null ? null : responseBody.byteStream();
