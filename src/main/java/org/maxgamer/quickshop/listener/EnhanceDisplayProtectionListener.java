...
public class EnhanceDisplayProtectionListener extends AbstractProtectionListener implements Listener {
    ...
    public void block(FurnaceSmeltEvent event) {
        ...
        if (AbstractDisplayItem.checkIsGuardItemStack(itemStack)) {
            ...
            if (furnace instanceof Furnace) {
                ...
                sendAlert(
                        "[DisplayGuard] Block  " + event.getBlock().getLocation() + " trying smelt with DisplayItem.");
                Util.inventoryCheck(furnace1.getInventory());
    ...
    public void entity(EntityPickupItemEvent e) {
        ...
        e.getItem().remove();
        sendAlert("[DisplayGuard] Entity " + e.getEntity().getName() + " # " + e.getEntity().getLocation()
    ...
    public void inventory(InventoryDragEvent event) {
        ...
        if (AbstractDisplayItem.checkIsGuardItemStack(itemStack)) {
            ...
            Util.inventoryCheck(event.getInventory());
            sendAlert(
                    "[DisplayGuard] Player  " + event.getWhoClicked().getName() + " trying use DisplayItem crafting.");
            return;
    ...
    public void player(PlayerBucketEmptyEvent event) {
        ...
        final Shop shop = getShopPlayer(waterBlock.getRelative(BlockFace.DOWN).getLocation(), true);
        if (shop == null) {
