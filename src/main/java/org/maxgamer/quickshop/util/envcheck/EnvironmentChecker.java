...
public final class EnvironmentChecker {
        ...
        for (Method declaredMethod : this.tests) {
            ...
            try {
                ...
                if (Arrays.stream(envCheckEntry.stage()).noneMatch(entry -> entry == stage)) {
                    ...
                    Util.debugLog("Skip test: " + envCheckEntry.name() + ": Except stage: "
                            + Arrays.toString(envCheckEntry.stage()) + " Current stage: " + stage);
                    continue;

                }

                if (false && !properties.containsKey("org.maxgamer.quickshop.util.envcheck.skip."
                ...
                switch (result) {
                    ...
                    case WARNING:
                        plugin.getLogger()
                                .warning("[WARN] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                        Util.debugLog("[Warning] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                        break;
                    case STOP_WORKING:
                        plugin.getLogger()
                                .warning("[STOP] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                        Util.debugLog(
                                "[Stop-Freeze] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                        // It's okay, QuickShop should continue executing checks to collect more data.
                        // And show user all errors at once.
                        break;
                    case DISABLE_PLUGIN:
                        plugin.getLogger()
                                .warning("[FATAL] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                        Util.debugLog(
                                "[Fatal-Disable] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                    ...
                    default:
                        plugin.getLogger().warning(
                                "[UNDEFINED] " + envCheckEntry.name() + ": " + executeResult.getResultMessage());
                ...
                if (executeResult != null) {
                    ...
                    results.put(envCheckEntry, new ResultContainer(CheckResult.SKIPPED,
                ...
                plugin.getLogger().log(Level.WARNING, "Failed to execute EnvCheckEntry [" + declaredMethod.getName()
                        + "]: Exception thrown out without getting caught. Something went wrong!", e);
    ...
    @SneakyThrows
    @EnvCheckEntry(name = "Signature Verify", priority = 0, stage =
    { EnvCheckEntry.Stage.ON_LOAD, EnvCheckEntry.Stage.ON_ENABLE })
        ...
        try {
                    ...
                    InputStream stream3 = loader.getResourceAsStream("META-INF/SELFSIGN.SF"))
            {
                ...
                if (stream1 == null || stream2 == null || stream3 == null) {
                    ...
                    plugin.getLogger().warning(
                            "The signature could not be found! The QuickShop jar has been modified or you're running a custom build.");
                    return new ResultContainer(CheckResult.KILL_SERVER,
                            "Security risk detected, QuickShop jar has been modified.");
            ...
            if (modifiedEntry.isEmpty()) {
                ...
                plugin.getLogger().severe(
                        "QuickShop detected that the jar has been modified! This is usually caused by the file being corrupted or virus infected.");
                plugin.getLogger().severe("To prevent severe server failure, QuickShop has been disabled.");
                plugin.getLogger().severe(
                        "For further information, Please join our support Discord server: https://discord.com/invite/bfefw2E.");
                return new ResultContainer(CheckResult.KILL_SERVER,
                        "Security risk detected, QuickShop jar has been modified.");
            ...
            plugin.getLogger().log(Level.WARNING,
                    "ALERT: QuickShop cannot validate itself. This may be caused by you having deleted QuickShop's jar while the server is running.",
                    ioException);
            return new ResultContainer(CheckResult.KILL_SERVER,
                    "Failed to validate digital signature! Security may be compromised!");
    ...
    @EnvCheckEntry(name = "Plugin Manifest Check", priority = 1, stage = { EnvCheckEntry.Stage.ON_LOAD,
    ...
    public ResultContainer manifestCheck() {
        ...
        if (!mainClass.equals("org.maxgamer.quickshop.QuickShop")) {
            ...
            return new ResultContainer(CheckResult.KILL_SERVER,
                    "Failed to validate main class! Security may be compromised!");
    ...
    @EnvCheckEntry(name = "Potential Infection Characteristics Check", priority = 1, stage = {
            EnvCheckEntry.Stage.ON_LOAD, EnvCheckEntry.Stage.ON_ENABLE })
    public ResultContainer potentialCheck() {
        ...
        try {
            ...
            if (found) {
                ...
                plugin.getLogger().log(Level.WARNING,
                        "ALERT: QuickShop detected Potential Infection, this jar may already infected by malware, stop the server and create full server backup immediately, run virus scan and contact the QuickShop support if you need!");
                return new ResultContainer(CheckResult.KILL_SERVER,
                        "Potential Infection detected, Killing server process...");
            ...
            plugin.getLogger().log(Level.WARNING,
                    ...
                    e);
            return new ResultContainer(CheckResult.KILL_SERVER,
                    "Failed to validate files in jar! Security may be compromised!");
    ...
    public ResultContainer spigotBasedServer() {
        ...
        try {
            ...
            try {
                ...
                plugin.getLogger().info("Running QuickShop-" + QuickShop.getFork() + " on NMS version " + nmsVersion
            ...
            plugin.getLogger().log(Level.WARNING, "Failed to check Spigot API!" + plugin.getServer().getBukkitVersion()
                    + " may not be Spigot based.");
            return failed;
    ...
    public ResultContainer coreSupportTest() {
        ...
        if (gameVersion == GameVersion.UNKNOWN) {
            ...
            return new ResultContainer(CheckResult.WARNING, "QuickShop may not fully support version " + nmsVersion
                    + "/" + ReflectFactory.getServerVersion() + ", Some features may not work.");
    ...
    public ResultContainer virtualDisplaySupportTest() {
        ...
        if (!gameVersion.isVirtualDisplaySupports()) {
            ...
            throwable = new IllegalStateException(
                    "This game version " + gameVersion + " not supports Virtual DisplayItem.");
            ...
            if (plugin.getServer().getPluginManager().getPlugin("ProtocolLib") != null) {
                ...
                return new ResultContainer(CheckResult.WARNING,
                        "ProtocolLib is not installed, virtual DisplayItem seems will not work on your server.");
        ...
        if (throwable != null) {
            ...
            plugin.getLogger().log(Level.SEVERE,
                    "Virtual DisplayItem Support Test: Failed to initialize VirtualDisplayItem", throwable);
            return new ResultContainer(CheckResult.WARNING,
                    "Virtual DisplayItem seems to not work on this Minecraft server, Make sure QuickShop, ProtocolLib and server builds are up to date.");
     ...
     * 
     * @EnvCheckEntry(name = "Permission Manager Test", priority = 10, stage =
     * EnvCheckEntry.Stage.ON_ENABLE) public ResultContainer
     * permManagerConflictTest() { if
     * (plugin.getServer().getPluginManager().isPluginEnabled("GroupManager")) {
     * return new ResultContainer(CheckResult.WARNING,
    ...
    public ResultContainer eolTest() {
        ...
        if (plugin.getGameVersion().isEndOfLife()) {
            ...
            return new ResultContainer(CheckResult.WARNING,
                    "End Of Life! This Minecraft version no-longer receive QuickShop future updates! You won't receive any updates from QuickShop, think about upgrading!");
