...
public class QuickShop extends JavaPlugin implements QuickShopAPI {
    ...
    private final List<ScheduledTask> timerTaskList = new ArrayList<>(3);
    @Getter
    ...
    /**
     * Return the QSRR's fork edition name, you can modify this if you want create
     * yourself fork.
     *
    ...
    private void load3rdParty() {
            ...
            if (this.openInvPlugin != null && openInvPlugin.isEnabled()) {
                ...
                try {
                    ...
                    // To avoid class conflict, we load the class from its class loader
                            openInvPlugin.getClass().getClassLoader().loadClass("com.lishid.openinv.IOpenInv")
                                    .isInstance(openInvPlugin))
                    {
        ...
        if (getConfig().getBoolean("plugin.PlaceHolderAPI")) {
            ...
            if (this.placeHolderAPI != null && placeHolderAPI.isEnabled()) {
                ...
                if (Util.verifyClassLoader(placeHolderAPI)
                        && Util.loadClassAndCheckName(placeHolderAPI, "me.clip.placeholderapi.PlaceholderAPIPlugin"))
                {
        ...
        if (getConfig().getBoolean("plugin.BlockHub.enable")) {
            ...
            if (this.blocksHubPlugin != null && blocksHubPlugin.isEnabled()) {
                ...
                if (Util.verifyClassLoader(blocksHubPlugin)
                        && Util.loadClassAndCheckName(blocksHubPlugin, "org.primesoft.blockshub.BlocksHubBukkit"))
                {
        ...
        if (getConfig().getBoolean("plugin.WorldEdit")) {
            ...
            if (this.worldEditPlugin != null && worldEditPlugin.isEnabled()) {
                ...
                if (Util.verifyClassLoader(worldEditPlugin)
                        && Util.loadClassAndCheckName(worldEditPlugin, "com.sk89q.worldedit.bukkit.WorldEditPlugin"))
                {
        ...
        if (getConfig().getBoolean("plugin.LWC")) {
            ...
            if (this.lwcPlugin != null && lwcPlugin.isEnabled()) {
                ...
                if (Util.verifyClassLoader(lwcPlugin)
                        && Util.loadClassAndCheckName(lwcPlugin, "com.griefcraft.lwc.LWCPlugin")
                        && Util.isMethodAvailable("com.griefcraft.lwc.LWC", "findProtection",
                                org.bukkit.Location.class))
                {
                    ...
                    getLogger().warning(
                            "Unsupported LWC version, please make sure you are using the modern version of LWC!");
                    this.lwcPlugin = null;
        ...
        if (getConfig().getBoolean("plugin.NBTAPI")) {
            ...
            if (nbtapi != null && nbtapi.isEnabled()) {
                ...
                if (Util.verifyClassLoader(nbtapi)
                        && Util.loadClassAndCheckName(nbtapi, "de.tr7zw.nbtapi.plugin.NBTAPI")
                        && Util.isMethodAvailable(nbtapi.getClass(), "isCompatible"))
                {
                    ...
                    if (!this.nbtapi.isCompatible()) {
                        ...
                        getLogger().warning(
                                "NBTAPI plugin failed to loading, QuickShop NBTAPI support module has been disabled. Try update NBTAPI version to resolve the issue. ("
                                        + nbtapi.getDescription().getVersion() + ")");
                        this.nbtapi = null;
                    ...
                    getLogger().warning(
                            "NBTAPI plugin is invalid, QuickShop NBTAPI support module has been disabled. Try update NBTAPI version to resolve the issue.");
        ...
        if (this.display) {
            ...
            if (AbstractDisplayItem.getNowUsing() == DisplayType.VIRTUALITEM) {
                ...
                if (protocolLibPlugin != null && Util.verifyClassLoader(protocolLibPlugin)
                        && protocolLibPlugin.isEnabled())
                {
            ...
            if (AbstractDisplayItem.getNowUsing() == DisplayType.REALITEM) {
                ...
                getLogger().warning(
                        "You're using Real Display system and that may cause your server lagg, switch to Virtual Display system if you can! (Checkout display-type option in config.yml)");
...
//    }
    ...
    public boolean loadEcon() {
        ...
        try {
            ...
            switch (EconomyType.fromID(getConfig().getInt("economy-type"))) {
                ...
                case UNKNOWN:
                    setupBootError(new BootError(this.getLogger(),
                            "Can't load the Economy provider, invaild value in config.yml."), true);
                    return false;
                case VAULT:
                        ...
                        try {
                            ...
                            if (!taxAccount.isEmpty()) {
                                ...
                                if (Util.isUUID(taxAccount)) {
                                    ...
                                    tax = PlayerFinder.findOfflinePlayerByUUID(PlayerFinder
                                            .findUUIDByName(Objects.requireNonNull(taxAccount), true, true));
                                ...
                                if (vault.isValid()) {
                                    ...
                                    if (!Objects.requireNonNull(vault.getVault()).hasAccount(tax)) {
                                        ...
                                        try {
                                            ...
                                            getLogger().warning(
                                                    "QuickShop detected tax account not exists, we're trying to create one. If you see any errors, please change tax-account in config.yml to server owner in-game username");
                                            if (vault.getVault().createPlayerAccount(tax)) {
                                                ...
                                                getLogger().warning(
                                                        "Cannot to create tax-account,  please change tax-account in config.yml to server owner in-game username");
                                        ...
                                        if (!vault.getVault().hasAccount(tax)) {
                                            ...
                                            getLogger().warning(
                                                    "Tax account's player never played this server before and failed to create one, that may cause server lagg or economy system error, you should change that name. But if this warning not cause any issues, you can safety ignore this.");
    ...
    public @NotNull Yaml getConfiguration() {
        ...
        return configurationForCompatibility == null
                ? configurationForCompatibility = LightningBuilder.fromFile(new File(getDataFolder(), "config.yml"))
                        .addInputStreamFromResource("config.yml").setReloadSettings(ReloadSettings.MANUALLY)
                        .setConfigSettings(ConfigSettings.PRESERVE_COMMENTS).createYaml()
    ...
    public final void onDisable() {
        ...
        Iterator<ScheduledTask> taskIterator = timerTaskList.iterator();
    ...
    public void reload() {
        ...
        try {
            ...
            if (getUpdateWatcher() != null) {
                ...
                if (updatedJar != null) {
                    ...
                    throw new IllegalStateException(
                            "Failed to reload QuickShop! Please consider restarting the server. (Plugin was updated)");
            ...
            if (!file.exists()) {
                ...
                throw new IllegalStateException(
                        "Failed to reload QuickShop! Please consider restarting the server. (Failed to find plugin jar)");
            ...
            if (throwable != null) {
                ...
                throw new IllegalStateException(
                        "Failed to reload QuickShop! Please consider restarting the server. (Plugin unloading has failed)",
                        throwable);
            ...
            if (plugin != null) {
                ...
                throw new IllegalStateException(
                        "Failed to reload QuickShop! Please consider restarting the server. (Plugin loading has failed)");
    ...
    private void initConfiguration() {
        ...
        try {
            ...
            getLogger().severe(
                    "Failed to save config.yml from jar, The binary file of QuickShop may corrupted. Please re-download from our website.");
        ...
        switch (resultReport.getFinalResult()) {
            ...
            case STOP_WORKING:
                ...
                if (command != null) {
                    ...
                    command.setTabCompleter(this); // Disable tab completer

                }
                break;
            case KILL_SERVER:
                getLogger().severe(
                        "[Security Risk Detected] QuickShop forcing crash the server for security, contact the developer for details.");
    ...
    private File writeSecurityReportToFile() {
        ...
        try {
            ...
            Files.write(new File(getDataFolder(), UUID.randomUUID() + ".security.letter.txt").toPath(),
                    environmentChecker.getReportMaker().bake().getBytes(StandardCharsets.UTF_8));
    ...
    public final void onEnable() {
        ...
        if (!this.onLoadCalled) {
            ...
            getLogger().severe(
                    "FATAL: onLoad not called and QuickShop trying patching them... Some Integrations will won't work or work incorrectly!");
            try {
        ...
        try {
            ...
            getLogger().warning(
                    "Because our error reporter doesn't work, please report this error to developer, thank you!");
        ...
        if (this.display && getConfig().getBoolean("shop.display-auto-despawn")) {
            ...
            // BUKKIT METHOD SHOULD ALWAYS EXECUTE ON THE SERVER MAIN THEAD // nuh uh
            timerTaskList.add(getServer().getAsyncScheduler().runAtFixedRate(this, this.displayAutoDespawnWatcher,
                    20 * 50, getConfig().getInt("shop.display-check-time") * 50L, TimeUnit.MILLISECONDS)); // not worth
                                                                                                           // async
                ...
                && (getConfig().getBoolean("shop.finding.exclude-out-of-stock")))
        {
            ...
            getLogger().severe(
                    "Shop find distance is too high with chunk loading feature turned on! It may cause lag! Pick a number under 100!");
        ...
        if (this.display && AbstractDisplayItem.getNowUsing() != DisplayType.VIRTUALITEM) {
            ...
            if (getDisplayItemCheckTicks() > 0) {
                ...
                timerTaskList.add(getServer().getAsyncScheduler().runAtFixedRate(this, t -> {
                    ...
                    for (Shop shop : getShopManager().getLoadedShops()) {
                        ...
                        getServer().getRegionScheduler().run(this, shop.getLocation(), task -> shop.checkDisplay());

                    }

                }, 1L, getDisplayItemCheckTicks() * 50L, TimeUnit.MILLISECONDS));
        ...
        timerTaskList.add(
                getServer().getAsyncScheduler().runAtFixedRate(this, signUpdateWatcher, 1, 500, TimeUnit.MILLISECONDS));
        // timerTaskList.add(getServer().getAsyncScheduler().runAtFixedRate(this,
        // shopContainerWatcher, 1, 250, TimeUnit.MILLISECONDS)); // Nobody use it

        if (logWatcher != null) {

            timerTaskList.add(
                    getServer().getAsyncScheduler().runAtFixedRate(this, logWatcher, 250, 250, TimeUnit.MILLISECONDS));
        ...
        if (getConfig().getBoolean("shop.ongoing-fee.enable")) {
            ...
            timerTaskList.add(getServer().getAsyncScheduler().runAtFixedRate(this, ongoingFeeWatcher, 1,
                    getConfig().getInt("shop.ongoing-fee.ticks") * 50L, TimeUnit.MILLISECONDS));
        ...
        if (offlinePlayers.length > 2000) {
            ...
            getServer().getAsyncScheduler().runNow(this,
                    task -> PlayerFinder.doLargeOfflineCachingWork(this, offlinePlayers));
    ...
    private boolean setupDatabase() {
        ...
        try {
            ...
            if (Objects.requireNonNull(dbCfg).getBoolean("mysql")) {
                ...
                dbCore = new MySQLCore(this, Objects.requireNonNull(host, "MySQL host can't be null"),
                        ...
                        Objects.requireNonNull(port, "MySQL port can't be null"), prefix, useSSL, optionsMap);
    ...
    private void submitMetrics() {
        ...
        if (!getConfig().getBoolean("disabled-metrics")) {
            ...
            metrics.addCustomChart(new Metrics.SimplePie("use_display_items",
                    () -> Util.boolean2Status(getConfig().getBoolean("shop.display-items"))));
            metrics.addCustomChart(
                    new Metrics.SimplePie("use_locks", () -> Util.boolean2Status(getConfig().getBoolean("shop.lock"))));
            metrics.addCustomChart(new Metrics.SimplePie("use_sneak_action",
                    () -> Util.boolean2Status(getConfig().getBoolean("shop.interact.sneak-to-create")
                            || getConfig().getBoolean("shop.interact.sneak-to-trade")
                            || getConfig().getBoolean("shop.interact.sneak-to-control"))));
            String finalEconomyType = economyType;
            metrics.addCustomChart(new Metrics.SimplePie("economy_type", () -> finalEconomyType));
            metrics.addCustomChart(new Metrics.SimplePie("use_display_auto_despawn",
                    () -> String.valueOf(getConfig().getBoolean("shop.display-auto-despawn"))));
            metrics.addCustomChart(new Metrics.SimplePie("use_enhance_display_protect",
                    () -> String.valueOf(getConfig().getBoolean("shop.enchance-display-protect"))));
            metrics.addCustomChart(new Metrics.SimplePie("use_enhance_shop_protect",
                    () -> String.valueOf(getConfig().getBoolean("shop.enchance-shop-protect"))));
            metrics.addCustomChart(new Metrics.SimplePie("use_ongoing_fee",
                    () -> String.valueOf(getConfig().getBoolean("shop.ongoing-fee.enable"))));
            metrics.addCustomChart(
                    new Metrics.SimplePie("database_type", () -> this.getDatabaseManager().getDatabase().getName()));
            metrics.addCustomChart(
                    new Metrics.SimplePie("display_type", () -> AbstractDisplayItem.getNowUsing().name()));
            ...
            metrics.addCustomChart(new Metrics.SingleLineChart("shops_created_on_all_servers",
                    () -> this.getShopManager().getAllShops().size()));
    ...
    private void updateConfig(int selectedVersion) throws IOException {
        ...
        if (selectedVersion == 121) {
            ...
            getConfig().set("shop.protection-checking-listener-blacklist",
                    Collections.singletonList("ignored_listener"));
            getConfig().set("config-version", ++selectedVersion);
        ...
        if (selectedVersion == 137) {
            ...
            boolean oldValueUntrusted = getConfig().getBoolean("integration.griefprevention.delete-on-untrusted",
                    false);
...
//            }
        ...
        if (selectedVersion == 153) {
            ...
            getConfig().set("shop.use-effect-for-potion-item",
                    getConfig().getBoolean("shop.shop.use-effect-for-potion-item", false));
        ...
        if (selectedVersion == 161) {
            ...
            getConfig().set("database.mysql-connect-options",
                    new ArrayList<>(Arrays.asList("autoReconnect=true", "useUnicode=true", "characterEncoding=utf8")));
            getConfig().set("config-version", ++selectedVersion);
        ...
        if (getConfig().getInt("matcher.work-type") != 0
                && GameVersion.get(ReflectFactory.getServerVersion()).name().contains("1_16"))
        {

            getLogger().warning(
                    "You are not using QS Matcher, it may meeting item comparing issue mentioned there: https://hub.spigotmc.org/jira/browse/SPIGOT-5063");

        }

        try (InputStreamReader buildInConfigReader = new InputStreamReader(
                new BufferedInputStream(Objects.requireNonNull(getResource("config.yml"))), StandardCharsets.UTF_8))
        {

            if (new ConfigurationFixer(this, new File(getDataFolder(), "config.yml"), getConfig(),
        ...
        try {
            ...
            if (new File(getDataFolder(), "messages.json").exists())
                Files.move(new File(getDataFolder(), "messages.json").toPath(),
                        new File(getDataFolder(), "messages.json.outdated").toPath());
            if (new File(getDataFolder(), "messages.json.outdated").exists()) {

                Files.write(new File(getDataFolder(), "about-messages.json.outdated.txt").toPath(),
                        Collections.singletonList(
                                "Please read v4 migrate guide there, after migration you can delete messages.json.outdated and this file: https://github.com/PotatoCraft-Studio/QuickShop-Reremake/wiki/Migrate:-v4-to-v5"));
