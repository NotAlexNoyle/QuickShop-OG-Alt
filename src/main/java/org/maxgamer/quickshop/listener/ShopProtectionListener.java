...
public class ShopProtectionListener extends AbstractProtectionListener {
        ...
        plugin.getServer().getWorlds().forEach(world -> {
            ...
            try {
                ...
                if (disableHopperMoveEvents) {
                    ...
                    plugin.getLogger().warning("World " + world.getName()
                    ...
                    plugin.getLogger().warning("If you still want keep enable disableHopperMoveEvents enables "
                ...
                if (disableHopperMoveEvents) {
                    ...
                    if (paperConfigYaml.exists()) {
                        ...
                        ConfigurationSection worldsSection = yamlConfiguration
                        ...
                        if (worldsSection != null) {
                            ...
                            if (checkAndEnableMoveEvent(perWorldSection)
                                    || checkAndEnableMoveEvent(defaultWorldSection))
                            {
                        ...
                        File paperConfigYamlNew = serverRoot.toPath().resolve("config")
                                .resolve("paper-world-defaults.yml").toFile();
                        if (paperConfigYamlNew.exists()) {

                            YamlConfiguration yamlConfiguration = YamlConfiguration
                                    .loadConfiguration(paperConfigYamlNew);
                        ...
                        File paperConfigPerWorldYamlNew = serverRoot.toPath().resolve(world.getName())
                                .resolve("paper-world.yml").toFile();
                        if (paperConfigPerWorldYamlNew.exists()) {

                            YamlConfiguration yamlConfiguration = YamlConfiguration
                                    .loadConfiguration(paperConfigPerWorldYamlNew);
                            if (checkAndEnableMoveEvent(yamlConfiguration)) {
                ...
                plugin.getLogger().log(Level.WARNING, "Failed to automatic disable disable-move-event for world ["
                        + world.getName() + "], please disable it by yourself or player can steal items from shops.",
                        ex);
    ...
    private boolean checkAndEnableMoveEvent(ConfigurationSection section) {
        ...
        if (section != null && section.isSet("hopper.disable-move-event")
                && section.getBoolean("hopper.disable-move-event"))
        {

            section.set("hopper.disable-move-event", false);
            section.set("hopper.disable-move-event-quickshop-tips",
                    "QuickShop automatic disabled this due it will allow other players steal items from shop. This notice only shown when have shops in current world and hopper protection is on and also disable-move-event turned on.");
            return true;
    ...
    // Prevent skulker box or shop sign being pushed, see
    // https://github.com/PotatoCraft-Studio/QuickShop-Reremake/issues/248
        ...
        for (int i = 0, a = e.blockList().size(); i < a; i++) {
            ...
            if (shop != null) {
                ...
                if (plugin.getConfig().getBoolean("protect.explode")) {
                    ...
                    plugin.logEvent(
                            new ShopRemoveLog(Util.getNilUniqueId(), "BlockBreak(explode)", shop.saveToInfoStorage()));
                    shop.delete();
    ...
    public void onBlockRedstoneChange(BlockRedstoneEvent event) {
        ...
        // plugin.getLogger().warning("[Exploit Alert] a Redstone tried to active of " +
        // shop);
        // Util.debugLog(ChatColor.RED + "[QuickShop][Exploit alert] Redstone was
        // activated on the
        // following shop " + shop);
    }
    ...
    public void onBlockSpread(BlockSpreadEvent e) {
        ...
        final Shop underBlockShop = getShopNature(newBlock.getRelative(BlockFace.DOWN).getLocation(), true);
    ...
    public void onStructureGrow(StructureGrowEvent event) {
        ...
        for (BlockState blockstate : event.getBlocks()) {
            ...
            // plugin.getLogger().warning("[Exploit Alert] a StructureGrowing tried to break
            // the shop of "
            // + shop);
            // Util.sendMessageToOps(ChatColor.RED + "[QuickShop][Exploit alert] A
            // StructureGrowing tried
