...
public class MsgUtil {
                    ...
                    if (msg.getHoverItemStr() != null) {
                        ...
                        try {
                            ...
                            if (data == null) {
                                ...
                                plugin.getQuickChat().sendItemHologramChat(player, msg.getMessage(player.getLocale()),
                                        data);
    ...
    public static String getGameLanguageCode(String languageCode) {
        ...
        if ("default".equalsIgnoreCase(languageCode)) {
            ...
            if (isLanguageEmpty && isCountryEmpty) {
                ...
                // plugin.getLogger().warning("Unable to get language code, fallback to en_us,
                // please change game-language option in config.yml.");
                languageCode = "en_us";
                ...
                if (isCountryEmpty || isLanguageEmpty) {
                    ...
                    // plugin.getLogger().warning("Unable to get language code, guessing " +
                    // languageCode + " instead, If it's incorrect, please change game-language
    ...
    public static String getTranslateText(ItemStack stack) {
        ...
        if (plugin.getConfig().getBoolean("force-use-item-original-name")) {
            ...
            return Util.getItemStackName(new ItemStack(stack.getType(), stack.getAmount()));
    ...
    public static String convertItemStackToTranslateText(Material mat) {
        ...
        return TextSplitter.bakeComponent(new BungeeQuickChat.BungeeComponentBuilder()
                .append(Util.getTranslateComponentForMaterial(mat)).create());
    ...
    public static void loadEnchi18n() {
        ...
        YamlConfiguration defaultYaml = YamlConfiguration.loadConfiguration(new InputStreamReader(
                Objects.requireNonNull(plugin.getResource("enchi18n.yml")), StandardCharsets.UTF_8));
        ...
        try {
            ...
            plugin.getLogger().log(Level.WARNING,
                    "Could not load/save transaction enchname from enchi18n.yml. Skipping...", e);
    ...
    public static void loadItemi18n() {
        ...
        YamlConfiguration defaultYaml = YamlConfiguration.loadConfiguration(new InputStreamReader(
                Objects.requireNonNull(plugin.getResource("itemi18n.yml")), StandardCharsets.UTF_8));
        ...
        try {
            ...
            plugin.getLogger().log(Level.WARNING,
                    "Could not load/save transaction itemname from itemi18n.yml. Skipping...", e);
    ...
    public static void loadPotioni18n() {
        ...
        YamlConfiguration defaultYaml = YamlConfiguration.loadConfiguration(new InputStreamReader(
                Objects.requireNonNull(plugin.getResource("potioni18n.yml")), StandardCharsets.UTF_8));
        ...
        try {
            ...
            plugin.getLogger().log(Level.WARNING,
                    "Could not load/save transaction potionname from potioni18n.yml. Skipping...", e);
    ...
    public static void loadTransactionMessages() {
                ...
                ResultSet rs = warpRS.getResultSet())
        {
            ...
            while (rs.next()) {
                ...
                List<ShopTransactionMessageContainer> msgs = OUTGOING_MESSAGES.computeIfAbsent(ownerUUID,
                        k -> new LinkedList<>());
     ...
     */
    @Deprecated
    public static void send(@NotNull UUID uuid, @NotNull ShopTransactionMessageContainer shopTransactionMessage,
            boolean isUnlimited)
    {
        ...
        if (!p.isOnline()) {
            ...
            plugin.getDatabaseHelper().saveOfflineTransactionMessage(uuid, shopTransactionMessage.toJson(),
            ...
            if (player != null) {
                ...
                if (hoverItemStr != null) {
                    ...
                    try {
                        ...
                        plugin.getQuickChat().sendItemHologramChat(player, shopTransactionMessage.getMessage(locale),
                                Objects.requireNonNull(Util.deserialize(hoverItemStr)));
     ...
     * @param uuid                            The uuid of the player to message
     * @param shopTransactionMessageContainer The message to send, if the given
     *                                        player are online it will be send
     *                                        immediately, Else, if they're not
     *                                        online, queues them in the database.
     */
    public static void send(@NotNull Shop shop, @NotNull UUID uuid,
            @NotNull ShopTransactionMessageContainer shopTransactionMessageContainer)
    {
        ...
        if (!p.isOnline()) {
            ...
            OUTGOING_MESSAGES.put(uuid, msgs);
            plugin.getDatabaseHelper().saveOfflineTransactionMessage(uuid, shopTransactionMessageContainer.toJson(),
                    System.currentTimeMillis());
            ...
            if (player != null) {
                ...
                if (hoverItemStr != null) {
                    ...
                    try {
                        ...
                        plugin.getQuickChat().sendItemHologramChat(p.getPlayer(),
                                shopTransactionMessageContainer.getMessage(locale),
                                Objects.requireNonNull(Util.deserialize(hoverItemStr)));
    ...
    public static void sendControlPanelInfo(@NotNull CommandSender sender, @NotNull Shop shop) {
        ...
        if ((sender instanceof Player) && !QuickShop.getPermissionManager().hasPermission(sender, "quickshop.use")
        ...
        if (!QuickShop.getPermissionManager().hasPermission(sender, "quickshop.setowner")) {
            ...
            chatSheetPrinter.printSuggestedCmdLine(
                    plugin.text()
                            .of(sender, "controlpanel.setowner",
                                    shop.ownerName() + ((plugin.getConfig().getBoolean(
                                            "shop.show-owner-uuid-in-controlpanel-if-op") && shop.isUnlimited())
                                                    ? (" (" + shop.getOwner() + ")")
                                                    : ""))
                            .forLocale(),
                    plugin.text().of(sender, "controlpanel.setowner-hover").forLocale(), "/qs setowner ");

        }

        // Staff
        if ((QuickShop.getPermissionManager().hasPermission(sender, "quickshop.staff")
                ...
                || QuickShop.getPermissionManager().hasPermission(sender, "quickshop.other.staff"))
        {
            ...
            chatSheetPrinter.printSuggestedCmdLine(
                    plugin.text().of(sender, "controlpanel.staff", shop.getStaffs().size()).forLocale(),
                    plugin.text().of(sender, "command.description.staff").forLocale(), "/qs staff ");
        ...
        if (QuickShop.getPermissionManager().hasPermission(sender, "quickshop.unlimited")) {
            ...
            String text = plugin.text().of(sender, "controlpanel.unlimited", bool2String(shop.isUnlimited()))
                    .forLocale();
            String hoverText = plugin.text().of(sender, "controlpanel.unlimited-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs("/qs silentunlimited {0}",
        ...
        if (QuickShop.getPermissionManager().hasPermission(sender, "quickshop.alwayscounting")) {
            ...
            String text = plugin.text()
                    .of(sender, "controlpanel.alwayscounting", bool2String(shop.isAlwaysCountingContainer()))
                    .forLocale();
            String hoverText = plugin.text().of(sender, "controlpanel.alwayscounting-hover").forLocale();
            String clickCommand = MsgUtil.fillArgs("/qs silentalwayscounting {0}",
                    shop.getRuntimeRandomUniqueId().toString());
            chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
                ...
                && QuickShop.getPermissionManager().hasPermission(sender, "quickshop.create.sell"))
        {
            ...
            if (shop.isSelling()) {
                ...
                String clickCommand = MsgUtil.fillArgs("/qs silentsell {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
                ...
                || shop.getOwner().equals(((OfflinePlayer) sender).getUniqueId()))
        {
            ...
            String text = MsgUtil.fillArgs(plugin.text().of(sender, "controlpanel.price").forLocale(),
                    (plugin.getConfig().getBoolean("use-decimal-format")) ? decimalFormat(shop.getPrice())
        ...
        if (QuickShop.getInstance().isAllowStack()) {
                            ...
                            && QuickShop.getPermissionManager().hasPermission(sender, "quickshop.create.changeamount"))
            {
                ...
                String text = plugin.text()
                        .of(sender, "controlpanel.stack", Integer.toString(shop.getItem().getAmount())).forLocale();
        ...
        if (!shop.isUnlimited()) {
            ...
            if (QuickShop.getPermissionManager().hasPermission(sender, "quickshop.refill")) {
                ...
                String text = plugin.text().of(sender, "controlpanel.refill", String.valueOf(shop.getPrice()))
                        .forLocale();
            ...
            if (QuickShop.getPermissionManager().hasPermission(sender, "quickshop.empty")) {
                ...
                String text = plugin.text().of(sender, "controlpanel.empty", String.valueOf(shop.getPrice()))
                        .forLocale();
                String hoverText = plugin.text().of(sender, "controlpanel.empty-hover").forLocale();
                String clickCommand = MsgUtil.fillArgs("/qs silentempty {0}",
                        shop.getRuntimeRandomUniqueId().toString());
                chatSheetPrinter.printExecutableCmdLine(text, hoverText, clickCommand);
        ...
        if (decimalFormat == null) {
            ...
            try {
                ...
                QuickShop.getInstance().getLogger().log(Level.WARNING,
                        "Error when processing decimal format, using system default: " + e.getMessage());
    ...
    public static void printEnchantment(@NotNull Player p, @NotNull Shop shop, ChatSheetPrinter chatSheetPrinter) {
        ...
        if (shop.getItem().hasItemMeta() && shop.getItem().getItemMeta().hasItemFlag(ItemFlag.HIDE_ENCHANTS)
                && plugin.getConfig().getBoolean("respect-item-flag"))
        {
    ...
    private static void printEnchantment(ChatSheetPrinter chatSheetPrinter, Map<Enchantment, Integer> enchs) {
        ...
        for (Entry<Enchantment, Integer> entries : enchs.entrySet()) {
            ...
            chatSheetPrinter.printLine(ChatColor.YELLOW + MsgUtil.getEnchi18n(entries.getKey()) + " "
                    + RomanNumber.toRoman(level == null ? 1 : level));
        ...
        for (String msg : messages) {
            ...
            try {
                ...
                if (spilledString == null) {
                    ...
                    builder.appendLegacyAndItem(spilledString.getLeft(), spilledString.getComponents(),
