...
public class SimplePriceLimiter implements PriceLimiter {
    ...
    public SimplePriceLimiter(double minPrice, double maxPrice, boolean allowFreeShop, boolean wholeNumberOnly,
            int maximumDigitsInPrice)
    {
    ...
    public SimplePriceLimiter(QuickShop plugin) {
        ...
        this(plugin.getConfig().getDouble("shop.minimum-price"), plugin.getConfig().getInt("shop.maximum-price"),
    ...
    public PriceLimiterCheckResult check(@NotNull ItemStack stack, double price) {
        ...
        SimplePriceLimiterCheckResult result = new SimplePriceLimiterCheckResult(PriceLimiterStatus.PASS, minPrice,
                maxPrice, price, maximumDigitsInPrice);

        if (Double.isInfinite(price) || Double.isNaN(price)) {

            return result.status(PriceLimiterStatus.NOT_VALID).priceShouldBe(minPrice);
        ...
        if (maximumDigitsInPrice != -1) {
            ...
            if (processedDouble.length > 1) {
                ...
                if (processedDouble[1].length() > maximumDigitsInPrice) {
                    ...
                    return result.status(PriceLimiterStatus.REACH_DIGITS_LIMIT).priceShouldBe(Double.parseDouble(
                            processedDouble[0] + "." + processedDouble[1].substring(0, maximumDigitsInPrice)));
        ...
        if (wholeNumberOnly) {
            ...
            try {
                ...
                return result.status(PriceLimiterStatus.NOT_A_WHOLE_NUMBER).priceShouldBe(Math.floor(price));
        ...
        if (!allowFreeShop || price != 0) {
            ...
            if (price < minPrice) {
                ...
                return result.status(PriceLimiterStatus.REACHED_PRICE_MIN_LIMIT).priceShouldBe(minPrice);

            }

            if (maxPrice != -1 && price > maxPrice) {

                return result.status(PriceLimiterStatus.REACHED_PRICE_MAX_LIMIT).priceShouldBe(maxPrice);
        ...
        if (materialLimit != null) {
            ...
            if (!allowFreeShop || price != 0) {
                ...
                if (perItemPrice < materialLimit.getKey()) {
                    ...
                    return result.status(PriceLimiterStatus.PRICE_RESTRICTED).priceShouldBe(materialLimit.getKey());

                } else if (perItemPrice > materialLimit.getValue()) {

                    return result.status(PriceLimiterStatus.PRICE_RESTRICTED).priceShouldBe(materialLimit.getValue());
