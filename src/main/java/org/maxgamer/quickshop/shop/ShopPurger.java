    ...
    private void run() {
        ...
        try {
            ...
            if (plugin.getConfig().getBoolean("purge.backup")) {
                ...
                plugin.getLogger().info("[Shop Purger] We have backup shop data as " + backupFileName
                        + ", if you ran into any trouble, please rename it to recovery.txt then use /qs recovery in console to rollback!");
            ...
            for (Shop shop : plugin.getShopManager().getAllShops()) {
                ...
                long noOfDaysBetween = ChronoUnit.DAYS.between(Util.getDateTimeFromTimestamp(lastPlayed),
                        Util.getDateTimeFromTimestamp(System.currentTimeMillis()));
                ...
                plugin.getServer().getAsyncScheduler().runNow(plugin, task -> {
                    ...
                    try {
                            ...
                            Util.runOnRegion(shop, () -> {
                                ...
                                if (returnCreationFee) {
                                    ...
                                    EconomyTransaction transaction = EconomyTransaction.builder()
                                            .amount(plugin.getConfig().getDouble("shop.cost")).allowLoan(false)
                                            .core(QuickShop.getInstance().getEconomy()).currency(shop.getCurrency())
                                            .world(shop.getLocation().getWorld()).to(shop.getOwner()).build();
                        ...
                        plugin.getLogger().info(
                                "[Shop Purger] Task completed, " + pendingRemovalShops.size() + " shops was purged");
