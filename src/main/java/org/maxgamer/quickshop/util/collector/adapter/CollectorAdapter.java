...
//                    break;
    ...
    public Map<?, ?> collectModules(@NotNull QuickShop plugin) {
        ...
        Map<Object, Object> data = new HashMap<>();
        data.put("item_matcher",
                plugin.getItemMatcher().getName() + "@" + plugin.getItemMatcher().getPlugin().getName());
        ...
        data.put("database_core", plugin.getDatabaseManager().getDatabase().getName() + "@"
                + plugin.getDatabaseManager().getDatabase().getPlugin().getName());
        data.put("gamelanguage_processor",
                MsgUtil.gameLanguage.getName() + "@" + MsgUtil.gameLanguage.getPlugin().getName());
        return data;
    ...
    public Map<?, ?> collectShopsInWorld(@NotNull QuickShop plugin) {
        ...
        plugin.getServer().getWorlds().forEach(world -> {
            ...
            perWorld.put("armor_stands",
                    world.getEntities().stream().filter(ArmorStand.class::isInstance).toArray().length); // DISPLAY
            perWorld.put("force_loaded_chunks", world.getForceLoadedChunks().size());
            perWorld.put("shops_in_force_loaded_chunks",
                    plugin.getShopManager().getShopsInWorld(world).stream()
                            .filter(shop -> world.getForceLoadedChunks().contains(shop.getLocation().getChunk()))
                            .toArray().length);
    ...
    public Map<?, ?> collectPlugins(@NotNull QuickShop plugin) {
        ...
        Arrays.stream(plugin.getServer().getPluginManager().getPlugins()).forEach(pl -> {
            ...
            perPlugin.put("is_addon", pl.getDescription().getDepend().contains("QuickShop")
                    || pl.getDescription().getSoftDepend().contains("QuickShop"));
...
//        map.put("qs_sentry", ); //TODO sentry log
    ...
    public Map<?, ?> collectShops(@NotNull QuickShop plugin) {
        ...
        map.put("database", plugin.getShopLoader().getOriginShopsInDatabase().stream()
                .map(ShopLoader.ShopRawDatabaseInfo::toString).toArray());
